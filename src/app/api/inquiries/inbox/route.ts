...
export async function GET(req: NextRequest) {
  ...
  if (!checkRateLimit(`inbox:${user.id}`, 20)) return rateLimitResponse();
  ...
  if (!['landlord', 'agent'].includes(profile.role)) return forbiddenResponse('Only landlords or agents can access the inbox');
  ...
  let query = supabase
    .from('inquiries')
    .select(
      `id, template_type, message, status, is_read_by_landlord, has_response, created_at, updated_at,
       property_id,
       property_listings!property_id(id, title, images),
       profiles!tenant_id(id, full_name, avatar_url)`,
      { count: 'exact' }
    )
  ...
  if (error) return serverErrorResponse('Failed to fetch inquiries');
  ...
  const inquiries = (data ?? []).map((row: Record<string, unknown>) => {
    ...
    return {
      ...
      message: row.message,
      status: row.status,
      is_read_by_landlord: row.is_read_by_landlord,
  ...
  return NextResponse.json({
    ...
    data: {
      ...
      new_inquiry_count: unreadCount ?? 0,
    },
  });
}
