...
export async function POST(request: NextRequest) {
  ...
  const { error: updateError } = await supabase
    .from('profiles')
    .update({ terms_accepted_at: acceptedAt, terms_accepted_version: version })
    .eq('id', user.id);
  ...
  return NextResponse.json({ success: true, data: { termsAcceptedAt: acceptedAt, termsAcceptedVersion: version } });
}
